...
FOLDER_NAME="$1"                                        #FOLDER NAME
MODULE_NAME="$2"                                        #MODULE NAME
MODULE_NAME_CAP="$3"                                    #APP NAME
MODEL_NAME="$4"                                         #MODEL UNDERSCORE NAME
RUN_SERVER="$5"                                         #RUN SERVER
DB_NAME="$6"                                            #DATABASE NAME
...
SECURITY_FILE="ir.model.access.csv"                     #SECRITY CSV FILE NAME
...
if [ "$RUN_SERVER" != "" ] && [ "$RUN_SERVER" == "run" ] && [ "$DB_NAME" != "" ]
then
    cd odoo/$FOLDER_NAME
    mkdir $MODULE_NAME
    ...
    cd community
    ./odoo-bin --addons-path=addons,../enterprise,../$FOLDER_NAME -d $DB_NAME
else
    cd odoo/$FOLDER_NAME
    mkdir $MODULE_NAME
